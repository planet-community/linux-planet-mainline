...
static int a3xx_hw_init(struct msm_gpu *gpu)
{
	...
	if (adreno_is_a304(adreno_gpu)) {
		gpu_write(gpu, REG_A3XX_VBIF_ROUND_ROBIN_QOS_ARB, 0x0003);
	}
	else if (adreno_is_a305(adreno_gpu)) {
	...
	/* Enable Clock gating: */
	if (adreno_is_a304(adreno_gpu) || adreno_is_a306(adreno_gpu))
	...
	if (adreno_is_a304(adreno_gpu) || adreno_is_a305(adreno_gpu) ||
